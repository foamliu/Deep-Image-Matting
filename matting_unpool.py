...
wl = tf.where(tf.equal(b_trimap, 128), tf.fill([train_batch_size, image_size, image_size, 1], 1.),
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    while epoch_num < max_epochs:
        while batch_num < batchs_per_epoch:
            ...
            batch_RGBs, batch_trimaps, batch_alphas, batch_BGs, batch_FGs, RGBs_with_mean = load_data(batch_alpha_paths,
                                                                                                      batch_eps_paths,
                                                                                                      batch_BG_paths)

            feed = {image_batch: batch_RGBs, GT_matte_batch: batch_alphas, GT_trimap: batch_trimaps,
                    GTBG_batch: batch_BGs, GTFG_batch: batch_FGs, raw_RGBs: RGBs_with_mean, training: True}
            ...
            if step % 200 == 0:
                ...
                test_RGBs, test_trimaps, test_alphas, all_shape, image_paths, trimap_size = load_alphamatting_data(
                    test_dir)
                vali_diff = []
